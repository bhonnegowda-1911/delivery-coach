...
import { generateFollowups, assessFollowupAnswer, type Followup, type FollowupAssessment } from '../lib/followups'
...
function Assessment({ data }: { data: FollowupAssessment }) {
...
type RowState =
  | { status: 'idle' }
  | { status: 'transcribing' }
  | { status: 'assessing'; transcript: Transcript }
  | { status: 'done'; transcript: Transcript; assessment: FollowupAssessment }
...
export default function FollowUps({ question, transcript }: { question: string; transcript: Transcript }) {
  ...
  async function generate() {
    ...
    try {
      ...
      setError((e as Error)?.message || 'Could not generate follow-ups.')
      setPhase('error')
    }
  }
