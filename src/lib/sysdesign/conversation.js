...
// computed once, at the end, by report.js.
...
function systemPrompt(stage, problem) {
...
function referenceFor(stage, problem) {
...
export function candidateDecisions(transcript = []) {
  return transcript
...
function priorContextSection(priorStages) {
...
function buildUserMessage(stage, transcript, latest, priorStages) {
  const lines = []
 ...
 * @param {object} args
 * @param {object} args.problem
 * @param {object} args.stage
 * @param {Array<{role:'candidate'|'interviewer', text:string}>} args.transcript  prior turns this stage
 * @param {Array<{label:string, decisions:string}>} [args.priorStages]  what was established in earlier stages
 * @param {string} args.message  the candidate's latest message
 * @param {string} args.anthropicKey
 * @param {string} [args.model]
 * @param {AbortSignal} [args.signal]
 * @returns {Promise<{reply:string, followUps:string[], aligned:boolean, coverage:object}>}
 */
export async function runStageTurn({
  ...
  const { parsed } = await chatStructured({
    ...
    user: buildUserMessage(stage, transcript, message, priorStages),
    schema: TURN_SCHEMA,
    signal,
  })
