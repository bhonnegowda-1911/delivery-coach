...
function buildUserMessage({ question, transcript, durationSec, filler }) {
  const lines = []
...
export function makeLlmAnalyzer(criteria = DEFAULT_CRITERIA) {
  return {
    id: criteria.id,
    ...
    async run(ctx) {
      ...
      const { parsed, raw } = await chatStructured({
        provider: 'anthropic',
